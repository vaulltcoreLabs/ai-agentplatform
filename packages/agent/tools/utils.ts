...
export async function getSandbox(
  ...
  // Phase 4.4: enforce the declared security policy at the point where tools
  // obtain their live sandbox. This is the single choke point for all tool
  // I/O — without it, a policy attached to the execution would never be
  // consulted because tools reconnect from serialized state.
  if (context.sandbox.securityPolicy) {
    return enforceSecurityPolicy(sandbox, context.sandbox.securityPolicy);
  }
  return sandbox;
}
