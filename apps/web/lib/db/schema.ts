...
// ---------------------------------------------------------------------------
// Artifact metadata — PostgreSQL is the AUTHORITATIVE source of truth for
// ownership, authorization, and lifecycle. The object body lives in R2; this
// table stores only metadata + the tenant-scoped object key. See
// docs/vaulltcore/phase5/storage-contract.md and packages/storage/artifact.ts.
// ---------------------------------------------------------------------------
...
export const artifacts = pgTable(
  ...
  (t) => ({
    pk: primaryKey({ columns: [t.tenantId, t.runId, t.artifactId] }),
    tenantRunIdx: index("artifacts_tenant_run_idx").on(t.tenantId, t.runId),
