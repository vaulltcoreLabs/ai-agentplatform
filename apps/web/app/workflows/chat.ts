...
import type { VaulltcoreAgentCallOptions } from "@vaulltcore/agent";
...
type Options = {
  ...
  modelId?: string;
  agentOptions?: Omit<VaulltcoreAgentCallOptions, "sandbox" | "skills">;
  assistantId?: string;
...
export async function runAgentWorkflow(options: Options) {
  ...
  let sandboxState: VaulltcoreAgentCallOptions["sandbox"]["state"] | undefined;
  ...
  try {
    ...
    const agentOptions: VaulltcoreAgentCallOptions = {
...
const runAgentStep = async (
  ...
  selectedModelId: string,
  modelId: string,
  agentOptions: VaulltcoreAgentCallOptions,
  stepNumber: number,
) => {
  "use step";
